# Mundial Predictor — Consulta técnica para Gemini

## Problema concreto
Modelo Poisson bivariado para predecir partidos del Mundial 2026.
France alpha_adj=1.52 vs Argentina alpha_adj=2.51.
Resultado: France 12.9% vs Argentina 68.6% — irreal.
Causa: Francia juega clasificatorias UEFA donde todos son duros y anota poco.
El modelo solo ve goles, no contexto histórico de calidad del equipo.

## Stack
Python, pandas, numpy, scipy (MLE L-BFGS-B), Streamlit.
Parámetros: alpha (ataque), beta (defensa), gamma (localía), 6 params de confederación.
Shrinkage adaptativo por n_partidos ya implementado.

## Validación
WC2022: 54.7% accuracy 1X2, 62.5% eliminatorias, Brier 0.61.

## Pregunta para Gemini
Queremos incorporar ELO ratings o FIFA Rankings como prior para 
inicializar los alpha de cada selección antes del MLE.

1. ¿Cuál es la forma matemáticamente correcta de incorporar un 
   prior ELO en un modelo Poisson MLE? ¿Como inicialización del 
   vector de parámetros, como regularización L2, o como prior 
   bayesiano explícito?

2. ¿Qué fuente de datos ELO recomendás para selecciones? 
   eloratings.net tiene CSVs históricos — ¿es suficiente o 
   hay algo mejor y gratuito?

3. ¿Cómo escalar ELO (rango ~1000-2100) a alpha (rango ~1.0-3.0) 
   sin distorsionar las diferencias relativas?

4. ¿Vale la pena la complejidad adicional dado que ya estamos en 
   54.7% accuracy con el modelo actual?

Mostrá código Python de ejemplo si es posible.

---

## Contexto adicional — CLAUDE.md completo del proyecto
...
| `monte_carlo(grupos, fuerzas, gamma, n, conf_ctx)` | Corre `n` iteraciones de `simular_torneo`. Devuelve DataFrame 48x5 con `equipo, campeon_%, final_%, semifinal_%, cuartos_%`, ordenado por campeon_%. |
...
- `France alpha_adj = 1.52` (sf=0.60 con 20 partidos, x UEFA=1.0)
- `Argentina alpha_adj = 2.51` (sf=0.60 con 29 partidos, x CONMEBOL=1.30)
